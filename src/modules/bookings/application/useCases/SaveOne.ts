...
export default class SaveOne {
    ...
    async exec(body: Booking, userLogged: Partial<User>) {
        ...
        const bookingCreated = await this.handleRelationBookingReservationAndSave(booking, reservation)
        return bookingCreated;
    }

    async addBookingInReservation(reservationToday: Reservation, booking: Booking){
        const reserved: ReservedHoursDto = { end: booking.hour_end ,start: booking.hour_start }
        reservationToday.reserved_hours.push(reserved)
        return await this.reservationRepository.save(reservationToday)
    }

    async handleRelationBookingReservationAndSave(booking: Booking, reservation: Reservation) {
